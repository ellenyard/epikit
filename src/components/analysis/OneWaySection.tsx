...
export function OneWaySection({ dataset }: OneWaySectionProps) {
  ...
  const exportToCSV = useCallback(() => {
    ...
    let csv = `# One-Way Tables Export\n`;
    csv += `# Timestamp: ${timestamp}\n`;
    csv += `# Denominator: ${percentMode === 'total' ? 'Total records' : 'Non-missing values'}\n`;
    csv += `# Variables: ${selectedVariables.map(v => dataset.columns.find(c => c.key === v)?.label || v).join(', ')}\n\n`;
    ...
    a.href = url;
    a.download = `one_way_tables_${new Date().toISOString().split('T')[0]}.csv`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
  }, [tableRows, dataset.columns, selectedVariables, percentMode]);
            ...
            <div className="bg-white border border-gray-200 rounded-lg p-4">
              ...
              {showPercentHelp && (
                <div className="mb-3 p-3 bg-gray-50 rounded-lg text-xs text-gray-700">
                  ...
                  <p className="mb-2">
                    <strong>% of Total Records:</strong> Use when missing data is meaningful (e.g., "refused to answer"). Missing values appear as a row.
                  </p>
                  <p>
                    <strong>% of Non-Missing:</strong> Use when missing values should be excluded (e.g., data entry errors). Percentages are based only on valid responses.
                  </p>
                </div>
              )}
